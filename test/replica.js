...
describe('Replica tests', function(){
    ...
    it('Propose operation', function(){
      ...
      replica.propose(op1);
      var ops = replica.operationsToProposeBySlot.get(0);
    ...
    it('Decision operation', function( done ){
        var op1 = {command_id:1,client_id:1,op:{name:'hello'}}
        var key = JSON.stringify({id:op1.command_id,client:op1.client_id});
        replica.decision(0 , op1).then(function(){
          replica.lastEmpltySlotInDecisions.should.be.exactly(1);
          var value = replica.operationsDecidedByOperation.get(key);
          value.should.be.exactly(0);
          replica.operationsDecidedBySlot.size.should.be.exactly(0);
          done();
        });
    });
    ...
    it('Decide operation with empty slots', function(done){
      ...
      replica.decision(1 , op1).then(function(){
        ...
        var value = replica.operationsDecidedByOperation.get(key1);
        value.should.be.exactly(1);
        replica.operationsDecidedBySlot.size.should.be.exactly(1);
        replica.decision(0 , op2).then(function(){
          replica.lastEmpltySlotInDecisions.should.be.exactly(2);
          var value = replica.operationsDecidedByOperation.get(key1);
          value.should.be.exactly(1);
          replica.operationsDecidedByOperation.get(key2).should.be.exactly(0);
    ...
    it('reproposeOperation' , function(  ){
        ...
        replica.operationsToProposeBySlot.set( 0 , new Set([op1]) )
        ...
        should.not.exists(replica.operationsToProposeBySlot.get(0))
        replica.operationsToProposeBySlot.get(1).values().next().value.command_id.should.be.exactly(1)
    });
    ...
    it('repropose same operation' , function( ){
        ...
        replica.operationsToProposeBySlot.get(0).values().should.be.empty()
    });
